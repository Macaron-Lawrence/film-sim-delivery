...
| `scripts/verify-delivery.py` | A delivered profile (plus optional source LUT, calibration, real images) to `verification.json` | `--profile`, `--lut`, `--calibration`, `--mode`, `--pre-gain`, `--protect`, `--images`, `--require-images`, `--allow-partial`, `--declare`, `--out` | Read-only except for the output JSON. Verdict is three-valued: `pass` (exit 0, everything required was recomputed and passed), `partial` (exit 3, computed checks passed but some metrics lacked input — **not a pass**), `fail` (exit 1). Image-level thresholds are two-stage: the absolute target applies when the supplied images meet it, otherwise a retention floor applies, recorded in `thresholds_used.basis`. |
...
- Calibration JSON records `mode` and `protect`, and the encoder **does** enforce both: using `--calibration` requires an explicit `--mode`, which must match the recorded mode; a `protect` mismatch, or an entry lacking `protect`/`pre_gain`, aborts generation before anything is written.
...
- Range validation now covers the values that produce visibly wrong output: `--divisions` (1–64), `--protect` (0 ≤ p < 1), `--pre-gain` (0.01–8.0) and `--strength` (0–1) are rejected at the entry point. Size values in the preview helpers and `--level` in `cube-to-hald.py` are still unchecked.
