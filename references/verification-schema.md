...
> 历史上这些示例值曾被误当作实测结果引用，所以它们已经换成中性占位值；
> 早期文档那组数字（只作为未归档的历史报告保留）见 `calibration.md` §0。
> 生成自己的文件请直接跑 `scripts/verify-delivery.py`，不要手写这些字段。

---

## 最小字段

> 下面每个数字都是**占位示例**（刻意不使用早期文档里那组数字，以免再次被误当成实测值）。
> 写你自己的文件时用当场算出来的值，最省事的办法是跑 `scripts/verify-delivery.py`。
...
```json
{
  ...
  "checks": {
    ...
    "mid_gray_out": 138.5,
    "white_out": 255.0,
    "brightness_ratio": 0.98,
    "top5pct_median": 242.0,
    "pct_ge_250": 2.9,
    "highlight_detail_std": 4.2
  },
