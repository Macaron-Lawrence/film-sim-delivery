...
- **修法**：走 `lut-to-ccprofile.py` 生成 XMP 创意配置文件 + wrapper 预设，装到
  `…/Adobe/CameraRaw/Settings/`。想要"叠加式预设"才用 `lut-to-xmp.py`（近似，保真 ~80%）。
...
- **实测跨度**：pre-gain **0.40 – 1.67**。共用必错。
- **修法**：`calibrate-luts.py` 逐卷二分求根；生成时用 `--calibration` 读入。

## 4. 不做护高光，纯白被压到 ~209

- **症状**："高光被切掉了/发白没细节"；实测最亮 5% 从 248 掉到 208，**≥250 的像素占比 0%**，
  高光区细节（std）从 6.1 掉到 1.0（只剩 1/6）。
- **根因**：胶片印片肩部的白点约 0.82，所有高光挤在一个天花板下。
- **修法**：`--protect 0.68`——按输入亮度加权，`L≤0.68` 用 LUT，`0.68→1.0` smoothstep 混回原始，
  `L=1.0` 时输出=输入。修完：最亮 5% 回到 247、≥250 占 3.3%、细节 5.1。
