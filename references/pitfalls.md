...
- **根因**：不同项目的 LUT 白点定位不同，所以没有可以共用的系数。
  早期文档报告过各来源的具体落点（spektrafilm 的"源白点 +4 档"与 0.3472、spectral_film_lut 的
  中灰区间 140–155、RT 集合 111–153、黑白卷 ~99），这些是**未归档的历史报告值，未纳入 CI**——
  见 `calibration.md` §0。**对当前输入必须重新测量**，不要把这些数字当成本次标定的依据。
